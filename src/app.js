...
const ICONS = {
  home: `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="m3.5 10.8 8.5-7 8.5 7v8.7a1.5 1.5 0 0 1-1.5 1.5h-4.5v-6h-5v6H5a1.5 1.5 0 0 1-1.5-1.5z" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>`,
  ...
  swap: `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M5 7h13m0 0-3-3m3 3-3 3M19 17H6m0 0 3 3m-3-3 3-3" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"/></svg>`,
  chart: `<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="4" y="12" width="3.5" height="8" rx="1.4" fill="none" stroke="currentColor" stroke-width="1.7"/><rect x="10.25" y="7" width="3.5" height="13" rx="1.4" fill="none" stroke="currentColor" stroke-width="1.7"/><rect x="16.5" y="4" width="3.5" height="16" rx="1.4" fill="none" stroke="currentColor" stroke-width="1.7"/></svg>`,
  calendar: `<svg viewBox="0 0 24 24" aria-hidden="true"><rect x="3" y="5" width="18" height="16" rx="3" fill="none" stroke="currentColor" stroke-width="1.8"/><path d="M7.5 3v4M16.5 3v4M3 10h18" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round"/><path d="M7.5 14h.01M12 14h.01M16.5 14h.01M7.5 17.5h.01M12 17.5h.01" stroke="currentColor" stroke-width="2.2" stroke-linecap="round"/></svg>`,
  target: `<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="11" cy="13" r="8" fill="none" stroke="currentColor" stroke-width="1.7"/><circle cx="11" cy="13" r="4" fill="none" stroke="currentColor" stroke-width="1.7"/><path d="m11 13 9-9m0 0h-5m5 0v5" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round"/></svg>`,
  report: `<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 3h9l4 4v13a1.5 1.5 0 0 1-1.5 1.5h-11A1.5 1.5 0 0 1 5 20V4.5A1.5 1.5 0 0 1 6.5 3z" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linejoin="round"/><path d="M14.5 3v5H19M8 17l2.5-3 2.3 1.8L16 11" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/></svg>`,
  settings: `<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="3.2" fill="none" stroke="currentColor" stroke-width="1.8"/><path d="M19.2 13.2a7.5 7.5 0 0 0 0-2.4l2-1.5-2-3.4-2.4 1a8 8 0 0 0-2-1.2L14.5 3h-5l-.3 2.7a8 8 0 0 0-2 1.2l-2.4-1-2 3.4 2 1.5a7.5 7.5 0 0 0 0 2.4l-2 1.5 2 3.4 2.4-1a8 8 0 0 0 2 1.2l.3 2.7h5l.3-2.7a8 8 0 0 0 2-1.2l2.4 1 2-3.4z" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linejoin="round"/></svg>`,
...
function defaultState() {
  return {
    ...
    settings: { sheetUrl: "", appsScriptUrl: "", lastSourceChangeAt: null, sourceHistory: [], hasPendingSync: false, lastSyncedAt: null },
    auditLog: []
  };
}
function loadState() {
...
function renderDashboard() {
  ...
  const series = [
    { key: "income", label: `Pemasukan ${periodLabel}`, value: Math.max(monthlyIncome, 0), colorClass: "masuk" },
    { key: "expense", label: `Pengeluaran ${periodLabel}`, value: Math.max(monthlyExpense, 0), colorClass: "keluar" },
    { key: "total", label: `Arus kas ${periodLabel}`, value: Math.max(monthlyCashflow, 0), colorClass: "total" }
  ];
...
function applySetupGateIfNeeded() {
  const needsSetup = !state.settings.sheetUrl || !state.settings.appsScriptUrl;
  ...
  gate.innerHTML = `<form id="firstSetupForm" class="setup-card"><div class="setup-heading"><div><span class="eyebrow">Satu kali setup</span><h2>Hubungkan penyimpananmu</h2></div><a class="tutorial-link" href="./tutorial.html" target="_blank" rel="noopener">Buka tutorial <span aria-hidden="true">↗</span></a></div><p>Masukkan link Google Sheet dan URL Web App Apps Script. Data tetap milikmu dan tersimpan di spreadsheet pribadi.</p><label>Link Google Sheet<input name="sheetUrl" placeholder="https://docs.google.com/spreadsheets/d/..." required></label><label>URL Apps Script<input name="appsScriptUrl" placeholder="https://script.google.com/macros/s/.../exec" required></label><div class="setup-actions"><button id="testSetupBtn" class="btn ghost" type="button">Uji Koneksi</button><button id="saveSetupBtn" class="btn" type="submit">Simpan & Lanjut</button></div><small id="setupState" class="setup-status">Belum ada koneksi.</small></form>`;
  document.getElementById("testSetupBtn").onclick = async () => {
    const form = document.getElementById("firstSetupForm");
  ...
  document.getElementById("firstSetupForm").onsubmit = async (e) => {
    ...
    const result = await saveDataSourceFromForm(fd);
    const message = document.getElementById("setupState");
    if (!result.ok) {
